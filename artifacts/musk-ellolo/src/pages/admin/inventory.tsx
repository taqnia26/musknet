...
type Sort = 'preferred' | 'name_asc' | 'name_desc' | 'quantity_asc' | 'quantity_desc' | 'value_desc';
type CreateForm = {
  nameAr: string; nameEn: string; displayNameAr: string; displayNameEn: string; invoiceNameAr: string; invoiceNameEn: string; descriptionAr: string; descriptionEn: string; sku: string; categoryId: string; price: string;
  openingQuantity: string; openingUnitCost: string; reorderPoint: string; targetStockQuantity: string;
  barcode: string; operationalType: 'finished_good' | 'raw_material' | 'packaging'; unitOfMeasure: string; inventoryNotes: string; openingLocationId: string; sellable: boolean; isActive: boolean;
};
const emptyCreate: CreateForm = { nameAr: '', nameEn: '', displayNameAr: '', displayNameEn: '', invoiceNameAr: '', invoiceNameEn: '', descriptionAr: '', descriptionEn: '', sku: '', categoryId: '', price: '0', openingQuantity: '0', openingUnitCost: '', reorderPoint: '5', targetStockQuantity: '20', barcode: '', operationalType: 'finished_good', unitOfMeasure: 'unit', inventoryNotes: '', openingLocationId: '', sellable: true, isActive: true };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] max-w-4xl overflow-y-auto">
        ...
        <div className="grid gap-3 rounded-lg border bg-muted/20 p-4 sm:grid-cols-4">
          <div><p className="text-xs text-muted-foreground">{t('الرصيد الحالي', 'Current')}</p><p className="text-xl font-bold">{formatInteger(currentStock, lang)}</p></div>
          <div><p className="text-xs text-muted-foreground">{t('حد الطلب', 'Reorder point')}</p><p className="text-xl font-bold">{formatInteger(item.reorderPoint, lang)}</p></div>
          <div><p className="text-xs text-muted-foreground">{t('المستهدف', 'Target')}</p><p className="text-xl font-bold">{formatInteger(item.targetStockQuantity, lang)}</p></div>
...
export default function AdminInventory({ titleKey = 'overview' }: { titleKey?: 'overview' | 'balances' }) {
  ...
  const [sort, setSort] = useState<Sort>('preferred');
  ...
  const { data, isLoading, isError } = useAdminListInventory({ search: search || undefined, categoryId: categoryId === 'all' ? undefined : Number(categoryId), stockStatus, sort: sort === 'preferred' ? undefined : sort });
  const createMutation = useAdminCreateInventoryProduct();
  const items = useMemo(
    () => sort === 'preferred' ? sortProductsForSelection(data?.items ?? [], lang) : data?.items ?? [],
    [data, sort, lang],
  );
  ...
  const createProduct = (event: React.FormEvent) => {
    event.preventDefault();
    const payload = { nameAr: create.nameAr.trim(), nameEn: create.nameEn.trim(), displayNameAr: create.sellable ? create.displayNameAr.trim() : create.displayNameAr.trim() || create.nameAr.trim(), displayNameEn: create.sellable ? create.displayNameEn.trim() : create.displayNameEn.trim() || create.nameEn.trim(), invoiceNameAr: create.sellable ? create.invoiceNameAr.trim() : create.invoiceNameAr.trim() || create.nameAr.trim(), invoiceNameEn: create.sellable ? create.invoiceNameEn.trim() : create.invoiceNameEn.trim() || create.nameEn.trim(), descriptionAr: create.descriptionAr.trim(), descriptionEn: create.descriptionEn.trim(), sku: create.sku.trim(), categoryId: Number(create.categoryId), price: create.sellable ? Number(create.price) : 0, openingQuantity: Number(create.openingQuantity), ...(create.openingUnitCost !== '' ? { openingUnitCost: Number(create.openingUnitCost) } : {}), reorderPoint: Number(create.reorderPoint), targetStockQuantity: Number(create.targetStockQuantity), barcode: create.barcode.trim() || null, operationalType: create.operationalType, unitOfMeasure: create.unitOfMeasure.trim(), inventoryNotes: create.inventoryNotes.trim(), sellable: create.sellable, isActive: create.isActive, ...(create.openingLocationId ? { openingLocationId: Number(create.openingLocationId) } : {}) };
    if ([payload.nameAr, payload.nameEn, payload.displayNameAr, payload.displayNameEn, payload.invoiceNameAr, payload.invoiceNameEn, payload.sku, payload.unitOfMeasure].some((value) => !value) || (create.sellable && (!payload.descriptionAr || !payload.descriptionEn)) || !categories.some((category) => category.id === payload.categoryId) || [payload.price, payload.openingQuantity, payload.openingUnitCost, payload.reorderPoint, payload.targetStockQuantity].some((value) => value != null && (!Number.isFinite(value) || value < 0)) || ![payload.openingQuantity, payload.reorderPoint, payload.targetStockQuantity].every(Number.isSafeInteger) || (payload.openingQuantity > 0 && payload.openingUnitCost == null) || (create.openingLocationId && !locations.some((location) => location.active && location.id === Number(create.openingLocationId)))) {
    ...
    if (existingProduct) {
      toast({ title: t('SKU مستخدم مسبقًا', 'SKU already exists'), description: t('استخدم رمزًا آخر أو عدّل الصنف الموجود.', 'Use another SKU or edit the existing item.'), variant: 'destructive' });
      return;
    }
  ...
  const cards = [
    { title: t('إجمالي الوحدات', 'Total units'), value: formatInteger(summary.totalUnits, lang), icon: Boxes },
    { title: t('قيمة المخزون', 'Inventory value'), value: <Money value={summary.totalValue} lang={lang} />, icon: CircleDollarSign },
    { title: t('منتجات منخفضة', 'Low stock'), value: formatInteger(summary.lowStockProducts, lang), icon: AlertTriangle },
    { title: t('منتجات نافدة', 'Out of stock'), value: formatInteger(summary.outOfStockProducts, lang), icon: XCircle },
  ];
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-3 rounded-xl border bg-card p-4 md:grid-cols-4"><div className="relative"><Search className="absolute start-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="ps-9" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t('بحث بالاسم أو SKU', 'Search name or SKU')} /></div><Select value={categoryId} onValueChange={setCategoryId}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">{t('كل التصنيفات', 'All categories')}</SelectItem>{categories.map((category) => <SelectItem key={category.id} value={String(category.id)}>{lang === 'ar' ? category.nameAr : category.nameEn}</SelectItem>)}</SelectContent></Select><Select value={stockStatus} onValueChange={(value) => setStockStatus(value as StockStatus)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">{t('كل الحالات', 'All statuses')}</SelectItem><SelectItem value="in_stock">{t('متوفر', 'In stock')}</SelectItem><SelectItem value="low">{t('منخفض', 'Low')}</SelectItem><SelectItem value="out">{t('نافد', 'Out')}</SelectItem></SelectContent></Select><Select value={sort} onValueChange={(value) => setSort(value as Sort)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="preferred">{t('الترتيب المعتمد للمنتجات', 'Preferred product order')}</SelectItem><SelectItem value="name_asc">{t('الاسم تصاعدياً', 'Name A-Z')}</SelectItem><SelectItem value="name_desc">{t('الاسم تنازلياً', 'Name Z-A')}</SelectItem><SelectItem value="quantity_asc">{t('الأقل كمية', 'Lowest quantity')}</SelectItem><SelectItem value="quantity_desc">{t('الأعلى كمية', 'Highest quantity')}</SelectItem><SelectItem value="value_desc">{t('الأعلى قيمة', 'Highest value')}</SelectItem></SelectContent></Select></div>
    <div className="overflow-x-auto rounded-xl border bg-card"><Table><TableHeader><TableRow><TableHead>{t('المنتج', 'Product')}</TableHead><TableHead>{t('التصنيف', 'Category')}</TableHead><TableHead>SKU</TableHead><TableHead>{t('الحالي', 'Current')}</TableHead><TableHead>{t('حد الطلب', 'Reorder')}</TableHead><TableHead>{t('المستهدف', 'Target')}</TableHead><TableHead>{t('القيمة', 'Value')}</TableHead><TableHead>{t('الحالة', 'Status')}</TableHead><TableHead className="sticky end-0 w-[190px] bg-card text-center">{t('الإجراءات', 'Actions')}</TableHead></TableRow></TableHeader><TableBody>{isLoading ? <TableRow><TableCell colSpan={9} className="py-12 text-center">{t('جاري التحميل...', 'Loading...')}</TableCell></TableRow> : isError ? <TableRow><TableCell colSpan={9} className="py-12 text-center text-destructive">{t('تعذر تحميل المخزون', 'Could not load inventory')}</TableCell></TableRow> : items.length === 0 ? <TableRow><TableCell colSpan={9} className="py-12 text-center text-muted-foreground">{t('لا توجد نتائج', 'No results')}</TableCell></TableRow> : items.map((item) => <TableRow key={item.id}><TableCell className="font-medium">{lang === 'ar' ? item.nameAr : item.nameEn}</TableCell><TableCell>{lang === 'ar' ? item.categoryNameAr : item.categoryNameEn}</TableCell><TableCell dir="ltr">{item.sku || '—'}</TableCell><TableCell className="font-bold">{formatInteger(item.stockQuantity, lang)}</TableCell><TableCell>{formatInteger(item.reorderPoint, lang)}</TableCell><TableCell>{formatInteger(item.targetStockQuantity, lang)}</TableCell><TableCell><Money value={item.inventoryValue} lang={lang} /></TableCell><TableCell><Badge variant={item.stockStatus === 'out' ? 'destructive' : 'outline'} className={item.stockStatus === 'in_stock' ? 'border-success text-success' : item.stockStatus === 'low' ? 'border-amber-500 text-amber-600' : ''}>{statusLabel(item.stockStatus, t)}</Badge></TableCell><TableCell className="sticky end-0 bg-card"><RowActions item={item} canEdit={canEdit} canDelete={canDelete} onPrintBarcode={printBarcode} categories={categories} /></TableCell></TableRow>)}</TableBody></Table></div>
    <Dialog open={createOpen} onOpenChange={setCreateOpen}>
      <DialogContent className="max-h-[92vh] max-w-2xl overflow-y-auto">
        <DialogHeader><DialogTitle>{t('إضافة صنف إلى المخزون', 'Add inventory item')}</DialogTitle></DialogHeader>
        <form onSubmit={createProduct} className="grid gap-4 sm:grid-cols-2">
          {([['nameAr', t('الاسم بالعربية', 'Arabic name')], ['nameEn', t('الاسم بالإنجليزية', 'English name')], ['sku', 'SKU']] as const).map(([key, label]) => <div key={key}><Label>{label}</Label><Input className="mt-1" required value={create[key]} onChange={(e) => setCreate((value) => ({ ...value, [key]: e.target.value }))} /></div>)}
          <div><Label>{t('نوع الصنف', 'Item type')}</Label><Select value={create.operationalType} onValueChange={(value: CreateForm['operationalType']) => setCreate((old) => ({ ...old, operationalType: value }))}><SelectTrigger className="mt-1"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="finished_good">{t('منتج نهائي', 'Finished good')}</SelectItem><SelectItem value="raw_material">{t('مادة خام', 'Raw material')}</SelectItem><SelectItem value="packaging">{t('عبوة', 'Packaging')}</SelectItem></SelectContent></Select></div>
          <div><Label>{t('وحدة القياس', 'Unit of measure')}</Label><Input className="mt-1" required value={create.unitOfMeasure} onChange={(e) => setCreate((old) => ({ ...old, unitOfMeasure: e.target.value }))} /></div>
          <div><Label>{t('الباركود GTIN', 'GTIN barcode')}</Label><Input className="mt-1" dir="ltr" inputMode="numeric" value={create.barcode} onChange={(e) => setCreate((old) => ({ ...old, barcode: e.target.value }))} /></div>
          <div><Label>{t('التصنيف', 'Category')}</Label><Select value={create.categoryId} onValueChange={(categoryId) => setCreate((old) => ({ ...old, categoryId }))}><SelectTrigger className="mt-1"><SelectValue placeholder={t('اختر التصنيف', 'Select category')} /></SelectTrigger><SelectContent>{categories.map((category) => <SelectItem key={category.id} value={String(category.id)}>{lang === 'ar' ? category.nameAr : category.nameEn}</SelectItem>)}</SelectContent></Select></div>
          <label className="flex items-center gap-2"><input type="checkbox" checked={create.sellable} onChange={(e) => setCreate((old) => ({ ...old, sellable: e.target.checked }))} />{t('قابل للبيع في المتجر', 'Sellable in storefront')}</label>
          <label className="flex items-center gap-2"><input type="checkbox" checked={create.isActive} onChange={(e) => setCreate((old) => ({ ...old, isActive: e.target.checked }))} />{t('نشط', 'Active')}</label>
          {create.sellable && <>
            {([['displayNameAr', t('اسم العرض بالعربية', 'Arabic display name')], ['displayNameEn', t('اسم العرض بالإنجليزية', 'English display name')], ['invoiceNameAr', t('اسم الفاتورة بالعربية', 'Arabic invoice name')], ['invoiceNameEn', t('اسم الفاتورة بالإنجليزية', 'English invoice name')]] as const).map(([key, label]) => <div key={key}><Label>{label}</Label><Input className="mt-1" required value={create[key]} onChange={(e) => setCreate((old) => ({ ...old, [key]: e.target.value }))} /></div>)}
            {([['descriptionAr', t('وصف المنتج بالعربية', 'Arabic product description')], ['descriptionEn', t('وصف المنتج بالإنجليزية', 'English product description')]] as const).map(([key, label]) => <div key={key} className="sm:col-span-2"><Label>{label}</Label><Textarea className="mt-1" required value={create[key]} onChange={(e) => setCreate((old) => ({ ...old, [key]: e.target.value }))} /></div>)}
            <div><Label>{t('سعر البيع', 'Selling price')}</Label><Input className="mt-1" type="number" min="0" step="0.01" value={create.price} onChange={(e) => setCreate((old) => ({ ...old, price: e.target.value }))} /></div>
          </>}
          {([['openingQuantity', t('الكمية الافتتاحية', 'Opening quantity')], ['openingUnitCost', t('تكلفة الوحدة الافتتاحية', 'Opening unit cost')], ['reorderPoint', t('حد إعادة الطلب', 'Reorder point')], ['targetStockQuantity', t('الكمية المستهدفة', 'Target quantity')]] as const).map(([key, label]) => <div key={key}><Label>{label}</Label><Input className="mt-1" type="number" min="0" step={key === 'openingUnitCost' ? '0.01' : '1'} value={create[key]} onChange={(e) => setCreate((old) => ({ ...old, [key]: e.target.value }))} /></div>)}
          <div><Label>{t('موقع الرصيد الافتتاحي', 'Opening stock location')}</Label><Select value={create.openingLocationId || 'default'} onValueChange={(value) => setCreate((old) => ({ ...old, openingLocationId: value === 'default' ? '' : value }))}><SelectTrigger className="mt-1"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="default">{t('الموقع الافتراضي', 'Default location')}</SelectItem>{locations.filter((location) => location.active).map((location) => <SelectItem key={location.id} value={String(location.id)}>{location.name}</SelectItem>)}</SelectContent></Select></div>
          <div className="sm:col-span-2"><Label>{t('ملاحظات الصنف', 'Item notes')}</Label><Textarea className="mt-1" value={create.inventoryNotes} onChange={(e) => setCreate((old) => ({ ...old, inventoryNotes: e.target.value }))} /></div>
          <Button className="sm:col-span-2" type="submit" disabled={createMutation.isPending}>{createMutation.isPending ? t('جاري الإنشاء...', 'Creating...') : t('إنشاء الصنف', 'Create item')}</Button>
        </form>
      </DialogContent>
    </Dialog>
